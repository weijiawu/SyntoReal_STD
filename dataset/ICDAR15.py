...
class ICDAR15(data.Dataset):
    def __init__(self, img_path, gt_path,negative_path, scale=0.25, length=512):
        super(ICDAR15, self).__init__()

        self.img_files = [os.path.join(img_path, img_file) for img_file in sorted(os.listdir(img_path))]
        self.pseudo_files = [os.path.join(gt_path, gt_file) for gt_file in sorted(os.listdir(gt_path))]
        self.pseudo_negative = [os.path.join(negative_path, gt_negative) for gt_negative in sorted(os.listdir(negative_path))]
    ...
    def __getitem__(self, index):
        with open(self.pseudo_files[index], 'r') as f:
            lines = f.readlines()

        vertices, labels = extract_vertices(lines)
        img = Image.open(self.img_files[index])

        negative = Image.open(self.pseudo_negative[index])

        # 判断是否pseudo-labe为空
        if vertices.shape[0] == 0:
            vertices = np.array([[0, 0, 0, 0, 0, 0, 0, 0]])
            labels = np.array([1])
            # print(labels_icdar15)

        # img_icdar15, vertices_icdar15 = adjust_height(img_icdar15, vertices_icdar15)
        # img_icdar15, vertices_icdar15 = rotate_img(img_icdar15, vertices_icdar15)
        # img_icdar15, vertices_icdar15 = crop_img(img_icdar15, vertices_icdar15, labels_icdar15, self.length)
        h, w = img.height, img.width
        ratio_w = self.length / w
        ratio_h = self.length / h
        vertices[:, [0, 2, 4, 6]] = vertices[:, [0, 2, 4, 6]] * ratio_w
        vertices[:, [1, 3, 5, 7]] = vertices[:, [1, 3, 5, 7]] * ratio_h

        img_icdar15 = img.resize((self.length, self.length), Image.BILINEAR)
        negative = negative.resize((self.length, self.length), Image.BILINEAR)
        transform = transforms.Compose([transforms.ColorJitter(0.5, 0.5, 0.5, 0.25), \
                                                transforms.ToTensor(), \
                                                transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))])
        score_map_icdar15, geo_map_icdar15, ignored_map_icdar15 = get_score_geo_target_source(img_icdar15,
                                                                                vertices,
                                                                                labels,
        ...
        return transform(img_icdar15), score_map_icdar15, geo_map_icdar15, ignored_map_icdar15
